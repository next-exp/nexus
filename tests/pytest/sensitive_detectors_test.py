...
@pytest.mark.first
def test_create_nexus_output_file(config_tmpdir, output_tmpdir, full_base_name, nexus_full_output_file):
...
/nexus/RegisterMacro {config_tmpdir}/{full_base_name}.config.mac
"""
     init_path = os.path.join(config_tmpdir, full_base_name+'.init.mac')
...
"""
     ...
     config_path = os.path.join(config_tmpdir, full_base_name+'.config.mac')
