...
namespace nexus {
  ...
  G4double UniformElectricDriftField::Drift(G4LorentzVector& xyzt)
  {
	...
	/// This must not be done with ionization clusters
	//	if (_cluster_per_step == false){
	position[i] = G4RandGauss::shoot(xyzt[i], transv_sigma);
	// } else {
	//   position[i] = xyzt[i];
	// }
      } else if (i == _axis) { // Longitudinal coordinate
    ...
    //   info->SetELLightYield(LightYield()*drift_length);
    // }
