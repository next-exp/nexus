...
SingleParticleAngle::SingleParticleAngle():
...
{
  msg_ = new G4GenericMessenger(this, "/Generator/SingleParticleAng/",
    "Control commands of single-particle generator.");

  msg_->DeclareMethod("particle", &SingleParticleAngle::SetParticleDefinition,
...
void SingleParticleAngle::SetParticleDefinition(G4String particle_name)
{
  particle_definition_ =
    G4ParticleTable::GetParticleTable()->FindParticle(particle_name);

  if (!particle_definition_)
    G4Exception("SetParticleDefinition()", "[SingleParticleAngle]",
      FatalException, "User gave an unknown particle name.");
}



void SingleParticleAngle::GeneratePrimaryVertex(G4Event* event)
{
...
G4double SingleParticleAngle::RandomEnergy() const
{
