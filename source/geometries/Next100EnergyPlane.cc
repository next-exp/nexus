...
Next100EnergyPlane::Next100EnergyPlane(G4LogicalVolume* mother_logic):
  ...
  _pmts_pitch (11.0 * cm)
{
  ...
  G4PVPlacement* _carrier_plate_physi = new G4PVPlacement(0, G4ThreeVector(0.,0.,carrier_plate_posz), carrier_plate_logic,
							  "CARRIER_PLATE", mother_logic, false, 0);
...
void Next100EnergyPlane::ReadParameters()
{
  const ParamStore& cfg_geom = ConfigService::Instance().Geometry();
  _visibility = cfg_geom.GetIParam("energy_plane_vis");
}
