...
namespace nexus {
  ...
  SiPM33NoCasing::SiPM33NoCasing(): GeometryBase(),
		    visibility_(1)

  {
    /// Messenger
    msg_ = new G4GenericMessenger(this, "/Geometry/NextNew/", "Control commands of geometry NextNew.");
    msg_->DeclareProperty("SiPM33NoCasing_vis", visibility_, "SiPM33NoCasing Visibility");
  }



  SiPM33NoCasing::~SiPM33NoCasing()
  {
  }



  G4ThreeVector SiPM33NoCasing::GetDimensions() const
  {
    return dimensions_;
  }
  ...
  void SiPM33NoCasing::Construct()
  {
    ...
    G4Box* sipm_solid = new G4Box("SiPM33NoCasing", sipm_x/2., sipm_y/2., sipm_z/2);

    G4Material* silicone = materials::OpticalSilicone();
    silicone->SetMaterialPropertiesTable(opticalprops::OptCoupler());

    G4LogicalVolume* sipm_logic =
      new G4LogicalVolume(sipm_solid, silicone, "SiPM33NoCasing");
    ...
    G4String sdname = "/SiPM33NoCasing/SiPM";
