...
namespace nexus {
  ...
  public:
    ...
    void WriteSensorTofInfo(int sensor_id, unsigned int time_bin, unsigned int charge);
    ...
    void WriteEventExtentInfo(int evt_number, unsigned int last_sns_data, unsigned int last_sns_tof, unsigned int last_hit, unsigned int last_particle);
    void WriteSensorPosInfo(unsigned int sensor_id, float x, float y, float z);

    size_t GetSnsDataIndex() const;
    size_t GetSnsTofIndex() const;
